...
def data_engineering_dag():
    ...
    _raw_table = aql.load_file(task_id=f'load_{raw_sources[0].split("/")[-1]}',
                               input_file=File(path=raw_sources[0]), 
                               output_table=raw_table,
                               use_native_support=True,
                               if_exists='replace')

    _raw_table = aql.load_file(task_id=f'load_{raw_sources[1].split("/")[-1]}',
    ...
    def feature_engineering(taxidf:pd.DataFrame):
        ...
        num_cols = pd.DataFrame(MaxAbsScaler().fit_transform(taxidf[['trip_distance']]), 
    ...
    _feature_file = aql.export_file(task_id='export', 
